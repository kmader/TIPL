  ...
  def asClassTag(imageType: Int) = imageType match {
     ...
     */
     ...
     def castArrToDouble(inVal: Any,imType: Int): Array[Double] = imType match {
              ...
              case IMAGETYPE_CHAR => inVal.asInstanceOf[Array[Byte]].map{_.doubleValue()}
              case IMAGETYPE_SHORT => inVal.asInstanceOf[Array[Short]].map{_.doubleValue()}
              case IMAGETYPE_INT => inVal.asInstanceOf[Array[Int]].map{_.doubleValue()}
              case IMAGETYPE_LONG => inVal.asInstanceOf[Array[Long]].map{_.doubleValue()}
              case IMAGETYPE_FLOAT => inVal.asInstanceOf[Array[Float]].map{_.doubleValue()}
     ...
     def fromDouble(inVal: Array[Double], outType: Int): Array[_] = outType match {
              case IMAGETYPE_BOOL => inVal.map{_>0}
              case IMAGETYPE_CHAR => inVal.map{_.byteValue}
              case IMAGETYPE_SHORT => inVal.map{_.shortValue}
              case IMAGETYPE_INT => inVal.map{_.intValue}
              case IMAGETYPE_LONG => inVal.map{_.longValue}
              case IMAGETYPE_FLOAT => inVal.map{_.floatValue}
              case IMAGETYPE_DOUBLE => inVal
              case _ => throw new IllegalArgumentException(outType+" is not a known image type")
              }
