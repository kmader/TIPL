...
public class ResizeBlock extends BaseTIPLBlock {
	...
	public ArgumentParser setParameterBlock(ArgumentParser p) {
		ArgumentParser t=fs.setParameter(p, prefix);
		if (t.getOptionBoolean("gui", "Use a GUI to set the parameters")) resizeGUI(t);
		
		
		return t;
	}
	public void resizeGUI(ArgumentParser p) {
		p.getOptionBoolean("showslice", "Show the slice and select the region using the ROI tool", new ArgumentList.ArgumentCallback() {

			@Override
			public Object valueSet(Object value) {
				String cValue=value.toString();
				if (Boolean.parseBoolean(cValue)) showSlicePreview();
				return value;
			}
		...
		//final TImg inputAim = TImgTools.ReadTImg(getFileParameter("input"),true, true);
	...
	public void showSlicePreview() {
		...
		ImageStack curStack=TImgToImageStack.MakeImageStack(inputAim);
		final ImagePlus curImage=new ImagePlus(getFileParameter("input"),curStack);
