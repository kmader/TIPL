...
public class VFilterScale extends FilterScale {
	...
	protected boolean runFilter(int bSlice, int tSlice) {
		...
		if (supportsThreading) System.out.println("Virtual (Slice-Based) Filter Running -- :<"+bSlice+","+tSlice+"> @ "+Thread.currentThread());
        ...
        for(int iz=lowz; iz<uppz; iz++) cachedSlices[iz-lowz]=null;
		...
		for(int oz=bSlice; oz<tSlice; oz++) {
            ...
            // This code is still pretty crappy because if there are overlapping slices it reloads them every time (2/3 scaling for example or simply filtering)
            ...
            /*cachedSlices=new Object[tiuppz-tilowz];
            for(int iz=tilowz; iz<tiuppz; iz++) {
                try {
                    cachedSlices[iz-tilowz]=_inputAim.PrivateSliceAccess(iz);
                } catch (Exception e) {
            ...
            for(int iz=lowz; iz<uppz; iz++) {
                if ((iz>=tilowz) && (iz<tiuppz)) { // a slice we want to cache
                    if (cachedSlices[iz-lowz]==null) { // not already in cache
                        try {
                            cachedSlices[iz-lowz]=_inputAim.getPolyImage(iz, inImageType);
                ...
                } else cachedSlices[iz-lowz]=null;
            }
            System.gc();
                    ...
                    if (scalingFilterGenerator==null) {
                        ...
                        if ((int) iposz!=zSliceLoaded) {
                            try {
                                ...
                                switch (inImageType) { // Only way to be sure the data is actually the right type
                                    case 10: // Boolean
                                        blSlice=(boolean[]) cachedSlices[(int) iposz-lowz];
                                        break;
                                    case 0: // Byte
                                        chSlice=(char[]) cachedSlices[(int) iposz-lowz];
                                        break;
                                    case 1: // Short
                                        shSlice=(short[]) cachedSlices[(int) iposz-lowz];
                                        break;
                                    case 2: // Integer
                                        itSlice=(int[]) cachedSlices[(int) iposz-lowz];
                                        break;
                                    case 3: // Float
                                        ftSlice=(float[]) cachedSlices[(int) iposz-lowz];
                                        break;	
                                    default:
                                        System.err.println("What sort of crap are you giving me:"+_inputAim.getImageType());
                                }
                        ...
                        final int sOff2 =  ((int) iposy)*dim.x +(int) iposx; // Offset in the current slice
                        switch (inImageType) { // Only way to be sure the data is actually the right type
                            case 10: // Boolean
                                if (blSlice[sOff2]) dcVox++;
                                break;
                            case 0: // Byte
                                dcVox=(chSlice)[sOff2];
                                break;
                        ...
                        for(int iz=max(lowz,tilowz); iz<min(tiuppz,uppz); iz++) {
                            try {
                                switch (_inputAim.getImageType()) { // Only way to be sure the data is actually the right type
                                    case 10: // Boolean
                                        blSlice=(boolean[]) cachedSlices[iz-lowz];
                                        break;
                                    case 0: // Byte
                            ...
                            for(int iy=max(lowy,tilowy); iy<min(tiuppy,uppy); iy++) {
                                //(iz*dim.y + iy)*dim.x + max(lowx,tilowx);
                                /*
                                 if (oz>lastSlice) {
                                 System.out.println("Slice:("+max(lowz,tilowz)+"-"+min(tiuppz,uppz)+")"+oz+", cop-in_sr:"+String.format("%.2f",((max(lowz,tilowz)*dim.y + iy)*dim.x + max(lowx,tilowx)+0.0)/1e6)+" MVx, _sp:"+String.format("%.2f",((min(tiuppz,uppz)*dim.y + iy)*dim.x + max(lowx,tilowx)+0.0)/1e6)+" MVx, out:"+String.format("%.2f",(ooff+0.0)/1e6)+" MVx, "+String.format("%.2f",(ooff*100.0)/inoff)+" %");
                                 lastSlice=oz;
                                 }
                                 */
                                int sOff2 =  (iy)*dim.x + max(lowx,tilowx) ; // Offset in the current slice
                                
                                for(int ix=max(lowx,tilowx); ix<min(tiuppx,uppx); ix++, sOff2++) {
                                    double dcVox=0;
                                    switch (_inputAim.getImageType()) { // Only way to be sure the data is actually the right type
                                        case 10: // Boolean
                                            if (blSlice[sOff2]) dcVox++;
                                    ...
                                    inSum+=dcVox;
                                    inCnt++;
                                    // The filter and coordinates apply in the input image dimensional space not the output.
                                    curFilter.addpt(ix,iposx,iy,iposy,iz,iposz,dcVox);
        ...
        if (scalingFilterGenerator!=null) filterNameOut=scalingFilterGenerator.make().filterName();
        
        String logAdd="VFilterScale Operation ("+filterNameOut+") : Upscale:("+upX+", "+upY+", "+upZ+"), Downscale ("+dnX+", "+dnY+", "+dnZ+"), T:"+eTime;
        logAdd+="\n InMean:"+String.format("%.2f",inSum/inCnt)+", OutMean:"+String.format("%.2f",outSum/outCnt)+", "+String.format("%.2f",outCnt/1e6)+" Mvx, "+String.format("%.2f",inCnt/1e9)+" Gop, "+String.format("%.2f",inCnt/outCnt)+" Op/Vx";
