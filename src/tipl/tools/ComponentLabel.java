...
public class ComponentLabel extends BaseTIPLPlugin {
	...
	public interface CLFilter {
		 ...
		 * 
		 * @param labelNumber
		 *            The label number of the given object
		 * @param voxCount
		 *            The voxel count of the given object
		 */
	...
	public final Integer zero = new Integer(0);
	public final Integer one = new Integer(1);
	...
	private Integer chase_up(int clab) {
		...
		if (cval.intValue() != clab)
			return chase_up(cval.intValue());
		else
			return cval;
	}
	...
	public boolean execute() {
		...
		for (int ir = 1; ir <= maxlabel; ir++) {
			if (objFilter.accept(ir, labelcounts.get(ir))) {
				labelremap.set(ir, new Integer(ir));
				objCount++;
			} else
	 ...
	 *            input template aim file
	 * @param elSize
	 *            the element size in the aim being considered
	 * @param minVolume
	...
	public TImg ExportMaskAimVoxels(TImgRO.CanExport templateAim,
		...
		final TImg outVirtualAim = ExportMaskAim(templateAim);
	...
	private int get_next_free_label(int lastlabel) {
		...
		for (int i = lastlabel; i <= maxlabel; i++) {
			if (labelcounts.get(i).intValue() == 0)
				return i;
		}
		return -1;
	}
	...
	public Object getInfo(final String request) {
		...
		if (sr.equals("avgcount")) {
			...
			for (int ir = 1; ir <= maxlabel; ir++) {
				final int cVal = labelcounts.get(ir).intValue();
				oVal += cVal;
				objCount += cVal > 0 ? 1 : 0;
			}

			return new Double(oVal / (objCount + ((objCount == 0) ? 1.0 : 0.0))); // dont
																					// divide
																					// by
																					// 0!
		}
	...
	private void remap_merge() {
		...
		for (int ij = maxlabel; ij > 0; ij--) {
			...
			if (reval != ij) {
				...
				final int newcount = labelcounts.get(reval.intValue());
				labelcounts.set(reval.intValue(), new Integer(oldcount
						+ newcount));
	...
	private void remap_merge_combine(int merges) {
		...
		for (int ij = 0; ij < aimLength; ij++) {
			if (labels[ij] > 0) {
				final int labRemapVal = labelremap.get(labels[ij]).intValue();
	 ...
	 * 
	 * @param minPct
	 *            Minimum percent to accept (0-100%)
	 * @param maxPct
	 *            Maximum percent to accept (0-100%)
	 */
	 ...
	 * 
	 * @param elSize
	 *            the size of one voxel in mm (stored in the aim header file)
	 * @param minVolume
	 *            the volume of the smallest object to be accepted
	 */
	 ...
	 *            the size of one voxel in mm (stored in the aim header file)
	 * @param minVolume
	 *            the volume of the smallest object to be accepted (in mm^3
	 * @param maxVolume
	 *            the volume of the largest object to be accepted
	 */
	...
	private void set_label(int pt, int applyLabel) {
		labelcounts.set(applyLabel, new Integer(labelcounts.get(applyLabel)
				.intValue() + 1));
		if (labels[pt] != 0)
			labelcounts.set(labels[pt], new Integer(labelcounts.get(labels[pt])
					.intValue() - 1));
