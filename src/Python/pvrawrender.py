...
pos=[int(cEle) for cEle in header[2].split(',')]
...
if doPos:
	imgIn.DataOrigin=[pos[0]-500,pos[1]-500,pos[2]-500]
...
def VolumeColoring(objRep,UseLogScale=False):
	...
	if makeDark: sof.Points=[minVal,0,0.5,0,maxVal,1,0.5,0]+[maxVal+0.1,0,0,0]
	...
	if filename.upper().find('CURV')>=0:
		...
		rVal=1/3
		lut.RGBPoints=[-1,0,1,1]+[-2*rVal,0,0.9,0.9]+[-rVal,0,0,0.9]+[0,0,0,0]+[rVal,0.90,0,0]+[2*rVal,0.9,0.9,0]+[1,1,1,1]
	...
	ScalarBarWidgetRepresentation1 = CreateScalarBar( Title='SAF', Enabled=1, LabelFontSize=12, LabelColor=[0.0, 0.0, 1.0], TitleFontSize=12, TitleColor=[0.0, 0.0, 1.0] )
...
#RenderView.CameraFocalPoint = [500,500,500]
...
print (RenderView.CameraPosition)
RenderView.WriteImage(filename+'2.png',"vtkPNGWriter",1)
