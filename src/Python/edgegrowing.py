...
fullset=lambda inlist: np.unique(map(lambda x: x[0],inlist)+map(lambda x: x[1],inlist)) # all elements in a dictionary / list
def run(objFile,edgeFile,objVars=['PCA3_X','PCA3_Y','PCA3_Z'],distEq=lambda x,y: abs(x[0]*y[0]+x[1]*y[1]+x[2]*y[2]),distThresh=5e-2):
	...
	grpLabels=dict(map(lambda x: (x,[x,extVars(objData.ldata[x]),-1]),objData.lkeys))
	joinList=[1]
...
def runEdges(objFile,edgeFile,objVars=['PCA3_X','PCA3_Y','PCA3_Z'],distEq=lambda x,y: abs(x[0]*y[0]+x[1]*y[1]+x[2]*y[2]),distThresh=5e-2):
	"""The main function in the script, lacFile and edgeFile are the input data files for the lacuna and the edge list respectively
	lacVars is the list of variables used for the distance metric that are to be saved in that order with the data
	distEq is the equation based on lacVars for computing the distance from lacuna x to lacuna y"""
	objData=tt.lacunData(objFile,rotMat=-1)
...
def swapGroups(grpLabels,edgeList,grpInfo,distFunc,distThresh):
	...
	for (i,j) in edgeList:
		...
		if (iGrp!=jGrp) & (iGrp>0) & (jGrp>0): # not already in the same group
			...
			if distItoImean<iVals[2]: newGrpLabels[i]=(newIgrp,iGrpVec,distItoImean)
			jVals=newGrpLabels.get(j,(jGrp,jGrpVec,distThresh*20))
			if distJtoJmean<jVals[2]: newGrpLabels[j]=(newJgrp,jGrpVec,distJtoJmean)
...
def joinToMap(joinList):
	...
	for (x,y) in joinList:
		...
		if (xdd is None) & (ydd is None): newLabels[min(x,y)]=[x,y]
		elif (xdd is not None): newLabels[xdd]+=[y]
		elif (ydd is not None): newLabels[ydd]+=[x]
	selfMap={}
	for (jnk,cVals) in newLabels.items():
		cMin=min(cVals)
		for cVal in cVals: selfMap[cVal]=cMin
	return selfMap
...
class AlignTensor():
	...
	def __init__(self,lacData,startList=None,objVars=['PCA3_X','PCA3_Y','PCA3_Z']):
		self.vars=objVars
		if lacData is not None: self.objCols=map(lambda x: lacData.lheader[x],objVars)
		if startList is None: self.list=[]
		else: self.list=startList
		self._cov=None
	def addpt(self,pt):
		self.list+=[pt]
		return self
...
def groupInfo(grpList,lacData,objVars=['PCA3_X','PCA3_Y','PCA3_Z']):
	...
	for (i,grpVals) in grpList.items():
		...
		grpInfo[gV]=addObjects(grpInfo.get(gV,(0,0,AlignTensor(lacData,objVars=objVars))),(1,colMapper(volCol),i)) # 
	
	return grpInfo
def writeOutput(outFile,lacData,grpList,colName='Group'):
	fOut=open(outFile,'w')
	fOut.write(lacData.preamble+'\n')
	ordCols=map(lambda y: dict(map(lambda x: (x[1],x[0]),lacData.lheader.items()))[y],sorted(lacData.lheader.values()))
	grpInfo=groupInfo(grpList,lacData)
	fOut.write(','.join(ordCols+[colName,colName+'_CNT',colName+'_VOL'])+'\n')
	for (i,grpVals) in grpList.items():
		fOut.write(','.join(lacData.ldata[i]+[str(grpVals[0]),str(grpInfo[grpVals[0]][0]),str(grpInfo[grpVals[0]][1])])+'\n')
	fOut.close()
