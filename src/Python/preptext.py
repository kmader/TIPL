...
class mergefiles(file):
	...
	def next(self):
		try:
			...
			print 'File '+str(self.files[self.curfile])+' is empty after '+str(self.nexts)+', moving along '+str(self.curfile+1)
			self.curfile+=1
			self.nexts=0
...
class posvec():
	...
	def add(self,cVec):
		...
		self.pts+=1
	def pos(self):
		if self.pts>0: return self.sumpos/self.pts
		else: return self.sumpos
class texttens():
	def __init__(self,icov=None,ipts=0,iminpts=1):
		if icov is None: icov=numpy.zeros((3,3))
		self.icov=icov
		self.cov=icov
		self.pts=ipts
		self.ptlist=[]
	...
	def logtens(self):
		...
		lw=numpy.eye(3)
		for i in range(3): lw[i,i]=tw[i]
		try: return numpy.dot(numpy.dot(self._v,lw),numpy.linalg.inv(self._v))
		except: 
	...
	def v1(self):
		self.tens()
		oV=self._v[:,2]
		if oV[0]<0: oV*=-1 # force all vectors to have a positive x1
		return oV 
	def trace(self):
...
def runTexture(ptxtFile,steps,lstep,dirV,isoSteps,useLstep,useBubbles,useRadPos,doStrain,keepExtremes=False,trimEdge=1,edges=None,minObj=2):
	#outFile=sys.argv[2]
	global allAlign,allAiso,allMean,allSum,allPts,allCov
	outFile='_text'.join(ptxtFile.split('_ptxt.csv'))+'.csv'
	outFile='_'.join(outFile.split(';'))
	if edges is None: edges=open(ptxtFile,'r')
	out=open(outFile,'w')
	def getfield(fd,txt):
		spos=txt.find(fd)
		slen=txt[spos:].find(';')
	...
	eheader=dict([(b,a) for (a,b) in enumerate([''.join(cline.upper().split('//')).strip() for cline in eheader])])
	...
	if useBubbles: 
		...
		for cLine in edges:
			try:
				...
				if not otext.has_key(cBub[0]): otext[cBub[0]]=texttens()
				if not otext.has_key(cBub[1]): otext[cBub[1]]=texttens()
				if not bubpos.has_key(cBub[0]): bubpos[cBub[0]]=posvec()
				if not bubpos.has_key(cBub[1]): bubpos[cBub[1]]=posvec()
			except:
	...
	outCols=['// POS_X','POS_Y','POS_Z','POS_R','POS_TH','COUNT','AISO','PCA1_X','PCA1_Y','PCA1_Z']
	for xc in 'XYZ':
		for yc in 'XYZ':
			outCols+=['TEXT_'+xc+yc]
	for xc in 'XYZ':
	...
	if useBubbles:
		for cKey in otext.keys():
			...
			procText(otext[cKey],'%f,%f,%f,%f,%f,' % tuple(list(iPos)+list(rPos[0:2])))
		...
		for i in range(steps[0]):
			for j in range(steps[1]):
				for k in range(steps[2]):
					...
					rPos=transPosRad(iPos)
					try:
						procText(otext[i][j][k],'%f,%f,%f,%f,%f,' % tuple(list(iPos)+list(rPos[0:2])))
					except:
						print str(otext[i][j][k])+' failed'
					
		print ('Steps',steps)			
	ctext=allAlign.tens()
	if allPts==0: allPts=1
	if allSum==0: allSum=0.1
	...
	return {'AvgTexture':avgTexture,'Groups':allPts,'PPG':allSum/allPts,'MeanAiso':allAiso/allSum,'GrMeanTr':allMean/allSum,'GrMeanAiso':allAlign.aiso,'GrMeanDir':allAlign.v1()}
	out.close()	
...
if __name__ == "__main__":
	...
	if len(elist)>0: edges=mergefiles(elist)
	else: edges=None
	if len(sys.argv)>2:
		...
		for i in range(sruns,fruns,3):
			steps=(i,i,i)
			steps=(0,i,0)
			resMat+=[runTexture(ptxtFile,steps,lstep,dirV,isoSteps,useLstep,useBubbles,useRadPos,doStrain,trimEdge=1,minObj=minObj)]
		print 'Final Results'
		for (cVal,cLine) in zip(range(sruns,fruns),resMat): print str(cVal)+':'+str(cLine)
	else:
		resMat=runTexture(ptxtFile,steps,lstep,dirV,isoSteps,useLstep,useBubbles,useRadPos,doStrain,trimEdge=0,edges=edges)
		print resMat
