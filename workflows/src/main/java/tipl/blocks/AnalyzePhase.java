...
public class AnalyzePhase extends BaseTIPLBlock {
    ...
    protected String getDescription() {
        return "Run component labeling, shape, density, and neigbhorhood analysis on a thresheld " +
                "phase";
    }
    ...
    public ArgumentParser setParameterBlock(final ArgumentParser p) {
        ...
        // sphKernelRadius=p.getOptionDouble("sphkernelradius",1.74,"Radius of spherical kernel
        // to use for component labeling: vertex sharing is sqrt(3)*r, edge sharing is sqrt(2)*r,
        // face sharing is 1*r ");
    ...
    /**
     * A class for the shape and neighbor analysis based on two easily overridden functions for
     * generating the voronoi and neighborhood calculating plugins
     *
     * @author mader
     */
    public static class ShapeAndNeighborAnalysis {
        ...
        public TImg[] execute(final TImgRO labeledImage, final TImgRO maskImage,
                              final boolean writeShapeTensor) {
         ...
         * @param obj
         * @param mask
         * @return a plugin object which when executed fills the mask with the obj (voronoi
         * transform or something like that)
         */
        public ITIPLPluginIO getGrowingPlugin(final TImgRO obj, final TImgRO mask) {
            ITIPLPluginIO KV = TIPLPluginManager.createBestPluginIO("kVoronoi", new TImgRO[]{obj,
                    mask});
