...
public class BaseBlockRunner implements IBlockRunner, ITIPLBlock {
    ...
    public ArgumentParser setParameter(final ArgumentParser p) {
        ...
        for (ITIPLBlock cBlock : blockList) {
            s = cBlock.setParameter(ioHelper.gatherIOArguments(s, cBlock.getPrefix(),
                    cBlock.getInfo()));
        }
    ...
    @Override
    public TypedPath getFileParameter(String cParm) {
        return ioHelper.getFileParameter(cParm);
    }
    ...
    public double memoryFactor() {
        System.out.println("Assuming the blockrunner requires just the maximum of its parts " +
                "(probably not true)");
        double maxFactor = 0;
        for (ITIPLBlock cBlock : blockList)
            if (cBlock.memoryFactor() > maxFactor) maxFactor = cBlock.memoryFactor();
        return maxFactor;
    }
    ...
    public long neededMemory() {
        ...
        for (ITIPLBlock cBlock : blockList)
            if (cBlock.neededMemory() > cneededMemory) cneededMemory = cBlock.neededMemory();
    ...
    public boolean isReady() {
        ...
        return firstBlock.isReady() && ioHelper.isReady(firstBlock.getPrefix(),
                firstBlock.getInfo());
    }
    ...
    public static void main(ArgumentParser p ) {
        ...
        final boolean simpleNames = p.getOptionBoolean("simplenames", false,
                "Use simple names (b1 instead of thresholdblock1");
        if (blocknames.length() > 0) {
            ...
            for (String iblockname : blocknames.split(",")) {
                ...
                try {
                    ...
                    if (simpleNames) {
                        cBlock.setPrefix(blockClassName.replaceAll("[^A-Z]",
                                "") + "B" + blockIndex + ":");
                    } else {
            ...
            p = cr.setParameter(p.subArguments("blocknames", true).subArguments("simplenames",
                    true));
            ...
            if (runAsJob)
                jobToRun = SGEJob.runAsJob(BaseBlockRunner.class.getName(), p.subArguments("gui",
                                true),
                        "sge:");
