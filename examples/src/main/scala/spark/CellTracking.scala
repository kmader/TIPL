...
import tipl.spark.{ShapeAnalysis, SparkGlobal}
import tipl.tools.GrayVoxels
import tipl.util.{D3float, D3int, TypedPath}

// ~/Dropbox/Informatics/spark/bin/spark-submit --class spark.images.CellTracking
// --executor-memory 4G --driver-memory 4G /Users/mader/Dropbox/tipl/build/TIPL_core.jar
// -tif=/Volumes/MacDisk/AeroFS/VardanData/ForKevin_VA/3D/Spheroid2/GFP
// sparksubmit --class spark.images.CellTrackingSingle /afs/psi.ch/project/tipl/jar/TIPL_core.jar
// -tif=/gpfs/home/mader/SpheriodData/3D/Spheroid2/GFP/  -@masternode=spark://merlinc60:7077
// -@sparkmemory=5G -cache -threshvalue=5000
/**
 * Class to hold the basic settings
 */
@serializable case class CellTrackingSettings(imgPath: TypedPath, savePath: TypedPath,
                                              checkpointResults: Boolean, cacheInput: Boolean,
                                              forcePartitions: Int, maxIters: Int,
                                              threshValue: Double)


// format for storing image statistics
case class CellImstats(min: Double, mean: Double, max: Double) extends Serializable
...
object CellTrackingCommon {
  def getParameters(args: Array[String]) = {
    ...
    val imgSuffix = p.getOptionPath("suffix", ".tif", "Suffix to be added to image path *.sss to " +
    ...
    val maxIters = p.getOptionInt("maxiters", Integer.MAX_VALUE, "Maximum number of iterations " +
      "for component labeling")
    val smallestObjSize = p.getOptionInt("smallestobj", 0, "Smallest objects to keep")

    (CellTrackingSettings(imgPath.append("/*" + imgSuffix), savePath, checkpointResults,
      cacheInput, forcePartitions, maxIters, threshValue), p)
  }

  // calculate statistics for an array
  def arrStats(inArr: Array[Double]) = CellImstats(inArr.min, inArr.sum / (1.0 * inArr.length),
    inArr.max)
...
object CellTracking {
  ...
  def main(args: Array[String]) {
    ...
    var loadedSlices = if (settings.forcePartitions > 0) doubleSlices.repartition(settings
      .forcePartitions) else doubleSlices
    ...
    val clImage = ImageTools2D.compLabelingBySlicePartJL(bwImage.mapValues {
      _.toIterator
    }, maxIters = settings.maxIters)
    println(clImage.mapValues { cSlice => cSlice.size}.collectAsMap.mkString("\t"))
    val sliceCLimage = clImage.map { inIter => (inIter._1.z, inIter._2.map { inVal => (inVal._1,
      inVal._2._1)})}
...
object CellTrackingSingle {
  ...
  def main(args: Array[String]) {
    ...
    bwImage.foreach {
      curSlice =>
        val clSlice = ImageTools2D.sliceCompLabel[Double](curSlice._2.toIterator, new D3int(1, 1,
          0), settings.maxIters)
