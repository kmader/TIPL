...
def StartDatabase(dbName='lacuna',mysql=True,debugMode=False):
    ...
    globals()['homeDb']=('pc7819.psi.ch','sage','test263',dbName)
...
def getSampleNum(cur,sampleName,projNum,tries=0):
    ...
    if (is_match < 1):
        try: 
            cur.execute('BEGIN; INSERT INTO SAMPLE (PROJECT_NUMBER,SAMPLE_AIM_NAME) VALUES (?,?); COMMIT',(projNum,sampleName))
        except:
            cur.execute('COMMIT')
...
def getProjNum(cur,projName,tries=0):
    ...
    if (is_match < 1):
        cur.execute('BEGIN; INSERT INTO PROJECT (PROJECT_NAME) VALUES (?); COMMIT',(projName,))
        try: 
            print 4
        except:
            cur.execute('COMMIT')
