...
useFancy = not p.getOptionBoolean(
    "usestandardthreshold",
    False,
    "Use standard threshold instead of the threshold with morphological operations",
)
if useFancy:
    Threshold = FancyThreshold
else:
...
p.getOptionD3float(
    "filt:elsize",
    D3float(0.743, 0.743, 0.743),
    "The default voxel size of the high resolution measurements",
)
p.getOptionBoolean(
    "filt:changeelsize", True, "The default element size is certainly wrong"
)
p.getOptionInt(
    "filt:filter", 1, "1 is gaussian and 4 is median"
)  # for second filter 3 is laplace
...
if useFancy:
    p.getOptionInt(
        "bodyt:closeiter", 3, "Body should be a mask like object close several times"
    )
if useFancy:
    p.getOptionBoolean("bodyt:clthresh", True, "Remove small components around body")
...
if useFancy:
    p.getOptionInt("lining:closeiter", 1, "Small objects which shouldnt be connected")
if useFancy:
    p.getOptionBoolean("lining:clthresh", True, "Remove small specs")
if useFancy:
    p.getOptionInt("lining:clminpct", 5, "Remove everything small than 10%")
...
if useFancy:
    p.getOptionInt("other:closeiter", 0, "Small objects which shouldnt be connected")
if useFancy:
    p.getOptionBoolean("other:clthresh", True, "Remove small specs")
if useFancy:
    p.getOptionInt("other:clminpct", 10, "Remove everything small than 10%")

# set default output names for threshold block
map(
    lambda blockName: p.getOptionPath(
        blockName + ":threshold", blockName + "_bw.tif", "Output file for " + blockName
    ),
    phaseBlocks,
)

gfiltName = p.getOptionPath(
    "filt:gfilt", "gfilt.tif", "Name for smooth-filtered output file"
)
# link filter to threshold
linkBlock = lambda blockName: p.getOptionPath(
    blockName + ":gfilt", gfiltName, "Automatically Fed Forward"
)
map(linkBlock, phaseBlocks)

# link threshold to analysis
linkBlockAP = lambda blockName: p.getOptionPath(
    blockName + "_ap:segmented",
    p.getOptionAsString(blockName + ":threshold"),
    "Automatically Fed Forward",
)
map(linkBlockAP, noMaskPhases)
# fill the mask in
linkBlockMask = lambda blockName: p.getOptionPath(
    blockName + "_ap:mask",
    p.getOptionAsString(maskPhase + ":threshold"),
    "Automatically Fed Forward",
)
map(linkBlockMask, noMaskPhases)

# set the labeled output files
linkBlockLabels = lambda blockName: p.getOptionPath(
    blockName + "_ap:labeled", blockName + "_lab.tif", "Automatically Fed Forward"
)
map(linkBlockLabels, noMaskPhases)
# set name of the phases
linkBlockLabels = lambda blockName: p.getOptionPath(
    blockName + "_ap:phase", blockName, "Automatically Fed Forward"
)
map(linkBlockLabels, noMaskPhases)

# forward the minimum voxel count to all phases
minVoxCount = p.getOptionInt("minvoxcount", 20, "Minimum voxel count")
linkBlockMVC = lambda blockName: p.getOptionInt(
    blockName + "_ap:minvoxcount", minVoxCount, "Automatically Fed Forward"
)
