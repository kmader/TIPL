...
elipFunc=lambda x0,y0,z0,a,b,c: lambda x,y,z: (((x-x0)/a)**2+((y-y0)/b)**2+((z-z0)/c)**2)<1
...
for z in range(0,imageSize.z,zSp)[1:-1]:
	for y in range(0,imageSize.y,ySp)[1:-1]:
		for x in range(0,imageSize.x,xSp)[1:-1]:
			...
			bubbleList.write('%f,%f,%f' % (x,y,z)+',%f,%f,%f' % (2*a,2*b,2*c)+',%f' %(4*3.14/3*a*b*c)+'\n')
bubbleList.close()
# Create the bubbles and plateau borders for every voxel in the image from the function
bubbleImg=zeros(int(imageSize.prod()),'i') # since it is number of bubbles each voxel belongs too (>0 means it is in a bubble)
...
UFOAM.main(["UFOAM","-stagelist=3,4,5,8","-outdir=.","-thickmap=distmap.tif"]) # fake thickness map for now
