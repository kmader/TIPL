...
for (i, v) in enumerate(blArr):  # Apply thresholds to every voxel in the image
    ...
    if plArr[i]:
        blKeepArr[i] = 1
        ptKeepArr[i] = 1
    elif v in keepBubbles:
        blKeepArr[i] = mapBubbles[v]  # apply check to each voxel (slow)
...
keepImgLabels.appendProcLog(
    "Filtered Bubbles: " + str(keepBubbles) + " and plateau borders as 1"
)
