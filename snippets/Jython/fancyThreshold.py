...
smallThickMin=options.getOptionDouble("smallradiusmin",1.0,"Smallest thickness for small objects")
smallThickMax=options.getOptionDouble("smallradiusmax",2.0,"Largest thickness for small objects")
mediumThickMin=options.getOptionDouble("mediumthickmin",smallThickMax,"Smallest thickness for medium objects")
mediumThickMax=options.getOptionDouble("mediumthickmax",10.0,"Largest thickness for medium objects")
largeThickMin=options.getOptionDouble("largethickmin",mediumThickMax,"Smallest thickness for large objects")
largeThickMax=options.getOptionDouble("largethickmax",20.0,"Largest thickness for large objects")
...
doCache=options.getOptionBoolean("cache","Cache mapped values (faster but more memory intensive")
...
if runAsJob: 
	scriptName=options.getOptionPath("sge:scriptname",os.path.abspath(inspect.getfile(inspect.currentframe())),"Path to Script File")
...
if runAsJob:
	job.submit()
	exit()
if doBG: import tipl.util.TImgTools.WriteBackground as SaveImage # allows background processing
inImg=ReadTImg(thickFile)
if doPreload: inImg.getShortAim() # preload the image
class keepSizedStructures(MappedImage.StationaryVoxelFunction):
    def __init__(self,minVal,maxVal,name):
        self.minVal=minVal
        self.maxVal=maxVal
        self.cname=name
    def name(self): return self.cname+": keep structures size:"+str((self.minVal,self.maxVal))
    def getRange(self): return MappedImage.typeRange(10) # output type is boolean
    def get(self,inval): return ((inval<self.maxVal) & (inval>self.minVal))
