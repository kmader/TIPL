...
tensdex = filter(
    lambda x: x[1], enumerate(map(lambda x: x.find("Inertial Tensor") >= 0, f))
)
...
# Read out raw data
cMRow = lambda x: map(
    lambda x: float(x), x[0][x[0].find("[") + 1 : x[0].find(",]")].split(",")
)
cUVec = lambda x: map(
    lambda x: float(x), x[1][x[1].find("[") + 1 : x[1].find(",]")].split(",")
)
...
def VolumeColoring(objRep, UseLogScale=False):
    ...
    sof.Points = (
        [minVal - 0.01, 0.0, 0.5, 0] + [minVal, 0.90, 0.5, 0] + [maxVal, 1, 0.5, 0]
    )  # second column means it's never transparent
    ...
    lut.RGBPoints = (
        ...
        + [minVal + 2 * rVal, 0.90, 0, 0]
        + [minVal + 3 * rVal, 0.9, 0.9, 0]
        + [maxVal, 1, 1, 1]
    )
    lut.VectorMode = 0
    ...
    ScalarBarWidgetRepresentation1 = CreateScalarBar(
        ...
        Enabled=1,
        LabelFontSize=12,
        LabelColor=[0.0, 1.0, 0.0],
        TitleFontSize=12,
        TitleColor=[0.0, 1.0, 0.0],
    )
