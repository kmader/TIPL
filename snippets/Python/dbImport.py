...
def getSampleNum(cur, sampleName, projNum, tries=0):
    if tries > 2:
        ...
        return -1
    cQry = cur.execute(
        "SELECT SAMPLE_AIM_NUMBER,SAMPLE_AIM_NAME FROM SAMPLE WHERE PROJECT_NUMBER=? AND SAMPLE_AIM_NAME LIKE ?",
        (int(projNum), sampleName),
    ).fetchall()
    print cQry
    if len(cQry) > 0:
        is_match = cQry[0][0]
    ...
    if is_match < 1:
        try:
            cur.execute(
                "INSERT INTO SAMPLE (PROJECT_NUMBER,SAMPLE_AIM_NAME) VALUES (?,?)",
                (projNum, sampleName),
            )
        except:
