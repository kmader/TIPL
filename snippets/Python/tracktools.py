...
## Variables needed
matchVars=['POS_X','POS_Y','POS_Z','VOLUME','MASK_DISTANCE_MEAN','PROJ_X','PROJ_Y','PROJ_Z','NEIGHBORS','PCA1_S','PCA2_S','PCA3_S']
matchVars=['POS_X','POS_Y','POS_Z','VOLUME','MASK_DISTANCE_MEAN','PROJ_X','PROJ_Y','PROJ_Z','PROJ_Z','PCA1_S','PCA2_S','PCA3_S']
...
## General Methods
matchFun=lambda bubPosVecA,bubPosVecB: sqrt(sum([(bubPosVecB[i]-bubPosVecA[i])**2 for i in (0,1,2)])) # simple euclidean distance funcion 
...
volCompare=lambda bubPosVec,bubPosMat: (abs((bubPosMat[:,3]-bubPosVec[3])/(bubPosMat[:,3]+bubPosVec[3]))) # compare volume between bubbles
maxVolDifference=0.5
volDifferencePenalty=0
groupMatchFun=lambda bubPosVec,bubPosMat: sqrt(sum((bubPosMat[:,0:3]-bubPosVec[0:3])*(bubPosMat[:,0:3]-bubPosVec[0:3]),1))+(volDifferencePenalty*(volCompare(bubPosVec,bubPosMat)>maxVolDifference) if volDifferencePenalty>0 else 0)

nhCompare=lambda bubPosVec,bubPosMat: abs(bubPosMat[:,8]-bubPosVec[8])
tRXPos=250
tRYPos=250
rCylPosDiff=lambda bubPosVec,bubPosMat: abs(sqrt((bubPosMat[:,0]-tRXPos)*(bubPosMat[:,0]-tRXPos)+(bubPosMat[:,1]-tRYPos)*(bubPosMat[:,1]-tRYPos))-sqrt((bubPosVec[0]-tRXPos)*(bubPosVec[0]-tRXPos)+(bubPosVec[1]-tRYPos)*(bubPosVec[1]-tRYPos)))
ZDiff=lambda bubPosVec,bubPosMat: abs(bubPosMat[:,2]-bubPosVec[2])

groupMatchFun=lambda bubPosVec,bubPosMat: sqrt(sum((bubPosMat[:,0:3]-bubPosVec[0:3])*(bubPosMat[:,0:3]-bubPosVec[0:3]),1))+1000*(volCompare(bubPosVec,bubPosMat)>maxVolDifference) # artifical weight if it is greater than 0.5
...
# New Version
tmatRead=lambda tmatFile: numpy.array(map(lambda x: map(lambda y: float(y),x.strip().split(',')),open(tmatFile).readlines()))
...
class chain:
	...
	def getChain(self,linkC):
		if self.mLinks.has_key(linkC): return self.mLinks[linkC]
		return -1
	def get(self):
		cntChain=dict([(a,0) for a in self.mLinks.values()])
		for a in self.mLinks.values(): cntChain[a]+=1
		return cntChain
class Memorize:
	def __init__ (self, f):
		self.f = f
...
class edgeData:
    def __init__(self,inFile):
        lacs=[(lacVal.split(',')[0:2],lacVal) for lacVal in open(inFile,'r').readlines()] # read file and format as lacun#,data
        self.correctPos=False
        self.fname=inFile
        self.preamble=lacs[0][1][:-1]
        self.lraw=[((int(cRow[0]),int(cRow[1])),cData[:-1].split(',')) for (cRow,cData) in lacs[2:] if len(cRow)>0]
        self.ldata=dict(self.lraw)
        lheader=lacs[1][1].split(',')
        self.lheader=dict([(b,a) for (a,b) in enumerate([''.join(cline.upper().split('//')).strip() for cline in lheader])])
        ...
        if self.correctPos:
        	ptTransform=lambda pt: (self.rotMat*matrix(pt).transpose(1,0)).transpose(1,0).tolist()[0][0:3]
        	self.getPosi=lambda nPos: array(ptTransform(self.getPosj(nPos)[0:3])+self.getPosj(nPos)[3:])
        else:
        	self.getPosi=lambda nPos: array(self.getPosj(nPos))
        self.lkeys=sorted(filter(lambda x: retainEdgeFunction(x[0],x[1],self.getPosi(x)),self.ldata.keys())) # only keep valid bubbles
        print ('Edges Filtered, Remaining',len(self.lkeys),'/',len(self.ldata.keys()),retainBubbleFunction,'Correction:',self.correctPos)
    ...
    def get(self,key):
    	if self.ldata.has_key(key): return self.ldata[key]
    	elif self.ldata.has_key((key[1],key[0])): return self.ldata[(key[1],key[0])]
    	return None
...
class lacunData:
    def __init__(self,inFile,rotMat=None,inCentRot=array([0,0,0])):
        ...
        lacs=[(lacVal[:lacVal.find(',')],lacVal) for lacVal in open(inFile,'r').readlines()] # read file and format as lacun#,data
        self.rotMat=matrix(numpy.eye(3))
        self.centRot=inCentRot
        self.correctPos=False
		
        if rotMat is -1: print 'Rotation has been force disabled!'
        elif rotMat is None:
        	try:
        		corData=tmatRead(inFile+'_tf.dat')
        		self.centRot=corData[0,:]
        		self.rotMat=matrix(corData[1:,:])
        		ptTransform=lambda pt: list(array((self.rotMat*matrix(list(array(pt)-self.centRot)).transpose(1,0)).transpose(1,0).tolist()[0][0:3])+self.centRot)
        ...
        self.fname=inFile
        self.preamble=lacs[0][1][:-1]
        self.lraw=[(int(cRow),cData[:-1].split(',')) for (cRow,cData) in lacs[2:] if len(cRow)>0]
        self.ldata=dict(self.lraw)
        lheader=lacs[1][1].split(',')
        self.lheader=dict([(b,a) for (a,b) in enumerate([''.join(cline.upper().split('//')).strip() for cline in lheader])])
        ...
        if self.correctPos:
        	...
        	self.getPosi=lambda nPos: array(self.getPosj(nPos))
        self.lkeys=sorted(filter(lambda x: retainBubbleFunction(self.getPosi(x)),self.ldata.keys())) # only keep valid bubbles
        print ('Bubbles Filtered, Remaining',len(self.lkeys),'/',len(self.ldata.keys()),retainBubbleFunction,'Correction:',self.correctPos)
        self.getPos=Memorize(lambda nPos: self.getPosi(nPos)) # cache bubble access
        self.posMat=zeros((len(self.lkeys),len(self.matchCols)))
        # precache values
        for (cRow,cKey) in enumerate(self.lkeys): 
    ...
    def fmatch(self,inFile,toLacun): # match using a file
    	...
    	for cLineText in inFile.readlines():
    		...
    		if cMatch[0]>=len(self.lraw): print ('Error',cMatch[0],' is too big for :',len(self.lraw))
    		elif cMatch[1]>=len(toLacun.lraw): print ('Error',cMatch[1],' is too big for :',len(toLacun.lraw))
    		else:
    			matchDict[self.lraw[cMatch[0]][0]]=(toLacun.lraw[cMatch[1]][0],0.0)
    	print matchDict
    	return matchDict
    def match(self,toLacun,bijTrim=True,maxDist=None,prescreenFun=lambda a,b: True,seedShift=lambda nPos: [0,0,0]): # bijTrim trims matches which are not bijective, max dist is the maximum distance
        matchDict={}
        for (cInd,cRow) in enumerate(self.posMat):
        	if prescreenFun(self.lkeys[cInd],cRow):
        		gRow=cRow
        		seedShiftV=seedShift(gRow)
        		for ssi in range(len(seedShiftV)): gRow[ssi]+=seedShiftV[ssi]
        ...
        if bijTrim:
        	mdict2=toLacun.match(self,bijTrim=False,maxDist=maxDist,seedShift=lambda oPos: [-1*ssd for ssd in seedShift(oPos)])
        	def bijMatch(cInd,mInd,fDict): # cur index, matching index foreign dict
        		if fDict.has_key(mInd):
        			return (fDict[mInd][0]==cInd)
        		else: return False
        	matchDict=dict([(a,b) for (a,b) in matchDict.items() if bijMatch(a,b[0],mdict2)])
        if maxDist is not None: matchDict=dict(filter(lambda a: a[1][1]<maxDist,matchDict.items()))
        return matchDict