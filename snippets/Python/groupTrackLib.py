...
def atan2(x,y): return 180/3.1415*radAtan2(x,y)
...
class dispVec:
	...
	def val(self,weighted=True):
		if self.count>0:
			if weighted: return self.wsum/self.weight
			else: return self.nsum/self.count
		else:
			return array([0,0,0])
	def val2(self,weighted=True):
		if self.count>0:
			if weighted: return self.wsum2/self.weight
			else: return self.nsum/self.count
		else:
			return array([0,0,0])
...
class groupTrack():
	...
	correctRot=False
	disableRot=False
	trackingFuncs={} # need to do these functions as a dictionary because python binds them all if you decalre them like parameters
	trackingFuncs['seedShift']=lambda lacA,lacB: lambda nPos: [0,0,0] # lip4 or coarsening foam
	...
	trackingFuncs['edgePathFunction']=lambda lacPath: '/'.join(lacPath[:-1]+['']) # for dkg21 and mi7
	centOfRot=[0,0,0]
	...
	def __init__(self,lacFileRoot,outFile=None,tlistFilterFunc=lambda x: True):
		...
		if outFile is None: outFile=lacFileRoot
	...
	def run(self):
		...
		if self.correctRot: # images are rotated
			def useRotMat(cFile):
				...
				if cNum%2==0: return makeRM(numpy.pi)
				else: return None
		else:
			useRotMat=lambda x: None # uses the matrices if they are present as clpor_2.csv_tf.dat files
		if self.disableRot:
		...
		inLine+=','+','.join(self.tracktoolbox.matchVars[3:]) # add standard columns
		if self.diffMatchVars: inLine+=',D_'+',D_'.join(self.tracktoolbox.matchVars[3:]) # add standard columns
		inLine+='\n'
		...
		if self.trimEdge>=1:
			...
			self.steps=(self.steps[0]-self.trimEdge,self.steps[1]-self.trimEdge,self.steps[2]-self.trimEdge)
		print (lmin,lmax,self.lstep,self.steps)
		# Init Radial Coordinates
		radCent=tuple(array(lacList[0].lmin)+(array(lacList[0].lmax)-array(lacList[0].lmin))/2) # immutable
		...
		for (cInd,cLacs) in enumerate(zip(lacList,lacList[1:]+[lacList[-1]])):
			(lacA,lacB)=cLacs
			try:
				frameCount=(trackingFuncs['getTimeStep'](lacB.fname)-trackingFuncs['getTimeStep'](lacA.fname))
			except:
				print 'Auto-frame count detection failed, default is : 1'
				frameCount=1
			cShift=lambda pos: frameCount*numpy.array(self.trackingFuncs['seedShift'](lacA,lacB)(pos))
			matchDict=lacA.match(lacB,prescreenFun=self.trackingFuncs['prescreenFun'],maxDist=frameCount*self.gMaxDist,bijTrim=self.isBijective,seedShift=cShift)
			...
			try:
				print ('Pretracking',self.trackingFuncs['getDataName'](lacA.fname),self.trackingFuncs['getDataName'](lacB.fname),'Seed-Dist',cShift([250,250,250]),'Mean-Dist:',mean(scoreMap),'Max-Dist:',max(scoreMap))
			except:
				print 'Tracking Failed!'
				continue
				
			for (a,b) in matchDict.items(): allChains.add((cInd,a),(cInd+1,b[0]))
		...
		nChains=dict(filter(lambda a: a[1]>self.minChainLength,tChains.items())) # only keep chains of a certain length
		print ('Chains',len(tChains),'/',len(nChains),'Mean Length',mean(tChains.values()),'/',mean(nChains.values()),'Max',max(tChains.values()))
		...
		edgeA=None
		matchDict=None
		for (cInd,cLacs) in enumerate(zip(lacList[0:-1],lacList[1:])): # frame number, (bubble, next bubble)
			(lacA,lacB)=cLacs
			try:
				frameCount=(trackingFuncs['getTimeStep'](lacB.fname)-trackingFuncs['getTimeStep'](lacA.fname))
			except:
			...
			if self.useMatchFile:
				...
				prescreenFunInChains=lambda x,pos: nChains.has_key(allChains.getChain((cInd,x)))
				cShift=lambda pos: frameCount*numpy.array(self.trackingFuncs['seedShift'](lacA,lacB)(pos))
				matchDict=lacA.match(lacB,prescreenFun=prescreenFunInChains,maxDist=frameCount*self.gMaxDist,seedShift=cShift)
			...
			self.tracktoolbox.retainEdgeFunction=lambda a,b,c: matchDict.has_key(a) & matchDict.has_key(b) # both bubbles are matched
			edgePA=edgeA
			...
			for (cEdge,edgePos) in nEdges:
				...
				if edgePA is not None:
					if (edgePA.backMap(matchDictPA,cEdge)==0): eInfo=1 # if it is created (not backmappable) make it 1
				if eInfo==0: 
					if (not edgeB.forwardMap(matchDict,cEdge)): eInfo=-1 # if it is deleted make it -1
				...
				outEdges.write('%i,%i,%i,%i,%f,%i,%f,%f,%f,%f,%f,%f,%f,%f' % (cInd,0,cEdge[0],cEdge[1],0,eInfo,edgeAvPos[0],edgeAvPos[1],edgeAvPos[2],erPos[0],erPos[1],edgeVec[0],edgeVec[1],edgeVec[2]))
				toStr=','+','.join(['%f']*(len(self.tracktoolbox.edgeVars)-3))+'\n'
			...
			for aLac in sorted(matchDict.keys()):
				...
				if (lacA.has_key(aLac) & lacB.has_key(bLac)):
					...
					outRaw.write('%i,%i,%i,%i,%i,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%i' % (cInd,curChain,curChainLength,aLac,bLac,mQual,avgPos[0],avgPos[1],avgPos[2],rPos[0],rPos[1],vecDir[0],vecDir[1],vecDir[2],nsVec[0],nsVec[1],nsVec[2],len(nEdges)))
					toStr=','+','.join(['%f']*(len(self.tracktoolbox.matchVars)-3))
					outRaw.write(toStr % tuple(lacA.getPos(aLac)[3:]))
					if self.diffMatchVars: outRaw.write(toStr % tuple(vecDir[3:]))
					outRaw.write('\n')
					# Do the lattice boxing / discretization
					avgPos=(aPos+bPos)/2
					cDex=numpy.int_(numpy.round((avgPos[0:3]-lmin)/self.lstep))				
					ncDex=max([cDex,[0,0,0]],0)
					ncDex=min([ncDex,[self.steps[0]-1,self.steps[1]-1,self.steps[2]-1]],0)
					if not self.keepExtremes:
						if ((ncDex[0]==cDex[0]) and (ncDex[1]==cDex[1]) and (ncDex[2]==cDex[2])):
							otext[cDex[0]][cDex[1]][cDex[2]].add(vecDir[0:3],self.trackingFuncs['wFun'](mQual),vec2=nsVec)
					else:
						otext[ncDex[0]][ncDex[1]][ncDex[2]].add(vecDir[0:3],self.trackingFuncs['wFun'](mQual),vec2=nsVec)
		
		
		# Write the result to the file
		print 'Mean Velocity Vector :'+str(dirSum/dirCnt)
		meanSubVec=lambda dvecObj: dvecObj.val()-(dirSum/dirCnt) # subtract the mean value
		...
		for i in range(self.steps[0]):
			for j in range(self.steps[1]):
				for k in range(self.steps[2]):
					...
					if 1:
						printVecLine(otext[i][j][k],'%i,%i,%i,%f,%f,%f,%f,%f,' % tuple([i,j,k]+list(iPos)+list(rPos[0:2])))
					#except:
		...
		for (cInd,cLacs) in enumerate(lacList):  outnames.write('%i,%s\n' % (cInd,cLacs.fname))
		outnames.close()
		# Continue RUnning
