...
def spy(
    ...
    if hack:
        ...
        p = subprocess.Popen(
            cmd, shell=shell, stdin=None, env=env, stdout=slave, close_fds=True, cwd=cwd
        )
        os.close(slave)
        line = ""
        while True:
...
tiplJars = [cJar for cJar in glob(tiplHome + "*.jar")] + [
    tiplHome + "Fiji.app/jars/ij.jar"
]
print tiplJars
tiplEnv = os.environ
# Setup Java VM
...
if getFreeMemory() < 32000:
    javaArgs = "-Xmx" + str(getFreeMemory()) + "M"

javaRuntime = tiplHome + "Fiji.app/java/" + javaver + "/jdk1.6.0_24/jre/bin/java"

if not tiplEnv.has_key("CLASSPATH"):
    tiplEnv["CLASSPATH"] = ""
...
def fetchTiplCommandOptions(option, opt_str, value, parser):
    ...
    def tiplParseArguments(cParser, funcOutput):
        ...
        for cOpt in filter(lambda x: len(x.strip()) > 0, funcOutput[argLine + 2 :]):
            ...
            cmdLetter = optName[0]
            while usedLetters.has_key(cmdLetter):
                cmdLetter = random.choice(string.letters)
    ...
    cOGroup = OptionGroup(
        parser,
        value,
        "Specific options for the given command which will be submitted with the program",
    )
...
optParse.add_option(
    ...
    action="store_true",
    dest="submit",
    help="Submit command to cluster ",
    default=False,
)
optParse.add_option(
    "-c",
    "--cmd",
    dest="cmd",
    help="Name of TIPL Command to Run [default: %default]",
    action="callback",
    default="UFOAM",
    callback=fetchTiplCommandOptions,
)
    ...
    if rPath.lower().find("slsbl") >= 0:  # coming from an NT mount somewhere
        ...
        if dataDrive == "Data10":
            rPath = (
                ...
                + eAccount
                + "/"
                + subDir[subDir.find("/") + 1 :]
            )
        else:
            rPath = "/sls/" + bLine + "/data/" + eAccount + "/" + subDir
    rPathList = rPath.split("/")
    if rPath.find("Data10"):
        globals()["userID"] = rPathList[rPathList.index("Data10") + 1]
    else:
        globals()["userID"] = rPathList[rPathList.index("data") + 1]
    if rPathList[-1].lower().find(".") >= 0:
        rPathList = rPathList[0:-1]
...
def checkForPath(cVar, cVal):
    ...
    if type(cVal) is type("bob"):
        if pathVariables.has_key(cVar):
            ...
            if cVal.find("/") >= 0:
                cPath = getGPFSPath(cVal)
            if cVal.lower().find(".tif") >= 0:
                cPath = getGPFSPath(cVal)
            if cVal.lower().find(".isq") >= 0:
                cPath = getGPFSPath(cVal)
    return cPath
...
if doSubmit:
    ...
    # Start DiCoClient
    command = (
        "java -jar DiCoClient.jar sEtUsErNaMe "
        + userID
        + "\; simu 1 all\; submitjob /work/sls/bin/tipl_batch.py -X "
        + logFile
        + " \;"
    )
    print command
