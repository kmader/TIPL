...
if gCurv: Curvature1.CurvatureType = 'Gaussian'
else: Curvature1.CurvatureType = 'Mean'
...
def VolumeColoring(objRep,UseLogScale=False):
	...
	if gCurv: 
		...
		lut.RGBPoints=[minVal,0,1,1]+[minVal+rVal,0,0.9,0.9]+[minVal+2*rVal,0,0,0.9]+[minVal+3*rVal,0,0,0]+[minVal+4*rVal,0.90,0,0]+[minVal+5*rVal,0.9,0.9,0]+[maxVal,1,1,1]
	objRep.LookupTable=lut
	objRep.Representation='Surface'
	
	ScalarBarWidgetRepresentation1 = CreateScalarBar( Title=meTitle, Enabled=1, LabelFontSize=12, LabelColor=[0.0, 0.0, 1.0], TitleFontSize=12, TitleColor=[0.0, 0.0, 1.0] )
