		...
		if len(ffiles)>0:	
			...
			if loadlac: 
				if len(lacFile)>0:
					...
					try:
						InsertCSV(cur,lacFile[0],samplename='_'.join(lacFile[0].split('/')[-3:-1]),projectTitle='Zihui')
					except:
			...
			if loadcan: 
				if len(canFile)>0: 
					try:
						InsertCSV(cur,canFile[0],samplename='_'.join(canFile[0].split('/')[-3:-1]),projectTitle='Zihui',tableName='Canal',CanalMode=1)
					except:
