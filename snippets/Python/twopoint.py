...
def selectSegment(aimImg,filt): # select segment applies a threshold (filt as lambda function) to the data
	img=aimImg.getIntAim() # read in the values as an integer array
	outdata=zeros(len(img),'z') # empty boolean image
	for (i,v) in enumerate(img): outdata[i]=filt(v) # apply filter to each voxel (slow)
