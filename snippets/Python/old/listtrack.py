...
if diffMatchVars: inLine+=',D_'+',D_'.join(tt.matchVars[3:]) # add standard columns
inLine+='\n'
...
for (cInd,cLacs) in enumerate(zip(lacList,lacList[1:]+[lacList[-1]])):
	...
	matchDict=lacA.match(lacB,prescreenFun=prescreenFun,maxDist=frameCount*gMaxDist,bijTrim=isBijective,seedShift=cShift)
	scoreMap=map(lambda a: a[1][1],matchDict.items())
	try:
		print ('Pretracking',getDataName(lacA.fname),getDataName(lacB.fname),'Seed-Dist',cShift([250,250,250]),'Mean-Dist:',mean(scoreMap),'Max-Dist:',max(scoreMap))
	except:
...
nChains=dict(filter(lambda a: a[1]>minChainLength,tChains.items())) # only keep chains of a certain length
...
for (cInd,cLacs) in enumerate(zip(lacList[0:-1],lacList[1:])): # frame number, (bubble, next bubble)
	...
	if useMatchFile:
		...
		cShift=lambda pos: frameCount*numpy.array(seedShift(lacA,lacB)(pos))
		matchDict=lacA.match(lacB,prescreenFun=prescreenFun,maxDist=frameCount*gMaxDist,seedShift=cShift)
	...
	tt.retainEdgeFunction=lambda a,b,c: matchDict.has_key(a) & matchDict.has_key(b) # both bubbles are matched
	edgePA=edgeA
	...
	for aLac in sorted(matchDict.keys()):
		...
		if (lacA.has_key(aLac) & lacB.has_key(bLac)):
			...
			if diffMatchVars: outRaw.write(toStr % tuple(vecDir[3:]))
			outRaw.write('\n')
			...
			if not keepExtremes:
				if ((ncDex[0]==cDex[0]) and (ncDex[1]==cDex[1]) and (ncDex[2]==cDex[2])):
					otext[cDex[0]][cDex[1]][cDex[2]].add(vecDir[0:3],wFun(mQual),vec2=nsVec)
			else:
				otext[ncDex[0]][ncDex[1]][ncDex[2]].add(vecDir[0:3],wFun(mQual),vec2=nsVec)
